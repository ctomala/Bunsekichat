...
class Flow(unittest.TestCase):
 ...
 def test_real_flow(self):
  ...
  with self.db.cursor() as x:
   x.execute("INSERT INTO users(username)VALUES('t'),('a'),('b')RETURNING id");t,s1,s2=[r[0]for r in x.fetchall()]
   x.execute("INSERT INTO analytic_plans(title,course,subject,course_level,parallel,shift)VALUES('p','TEST-COHORT','Materia Sintética','N1','A','Nocturna')RETURNING id");p=x.fetchone()[0]
  ...
  a.approve_question_bank_item(d[0]['id'],t);a.approve_question_bank_item(d[1]['id'],t);a.reject_question_bank_item(d[2]['id']);a.delete_question_bank_item(d[3]['id'])
